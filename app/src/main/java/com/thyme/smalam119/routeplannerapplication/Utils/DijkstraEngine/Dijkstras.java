package com.thyme.smalam119.routeplannerapplication.Utils.DijkstraEngine;
...
public class Dijkstras {
	...
	static private Character intToCharacter(int value) {
		return String.valueOf(value).charAt(0);
	}

	static private Graph distanceMatrixToGraph(int[][] distanceMatrix, int start, int end) {
		//  0 1 2 3
		//0 - a b c
		//1 a - d n
		//2 b d - x
...
//		g.addVertex('H', Arrays.asList(new Vertex('E', 1), new Vertex('F', 3)));
		...
		for (int y = 0; y < distanceMatrix.length; y++) {
			logBuilder.append("\n");
			Character from = intToCharacter(y);
			logBuilder.append(String.format("Edge=%c\n", from));
			...
			for (int x = 0; x < distanceMatrix[y].length; x++) {
				...
				Character to = intToCharacter(x);
				vertexes.add(new Vertex(to, distanceMatrix[y][x]));
				logBuilder.append(String.format("Add vertex(%c, %c)=%d\n", from, to, distanceMatrix[y][x]));
			}
			g.addVertex(from, vertexes );
		}

		Log.d("Dijkstras", logBuilder.toString());
		return g;
	}

	public static ArrayList<Integer> getBestPath(int[][] distanceMatrix, int start, int end) {
		Graph g = distanceMatrixToGraph(distanceMatrix, start, end);
		Log.d("getBestPath", String.format("start(i:%d, c:%c)", start, intToCharacter(start)));
		Log.d("getBestPath", String.format("end(i:%d, c:%c)", end, intToCharacter(end)));
		List<Character> charSolution = g.getShortestPath(intToCharacter(start), intToCharacter(end));

		ArrayList<Integer> intSolution = new ArrayList<Integer>();
		for (Character c : charSolution) {
			int numEquivalent = c - '0'; // a hack to convert char number to int equivalent
			intSolution.add(numEquivalent);
		}
